...
export async function browser(dist, { fixtures = new Map(), appAssetFailures = [], blockedProviderPatterns = [] } = {}){
 ...
 if(m.method==='Network.requestWillBeSent'){requestUrls.set(m.params.requestId,m.params.request.url);if(m.params.type==='Fetch')requests++;}
 if(m.method==='Network.loadingFailed'){networkFailures.push({url:requestUrls.get(m.params.requestId)||'',errorText:m.params.errorText,blockedReason:m.params.blockedReason,corsErrorStatus:m.params.corsErrorStatus});requestUrls.delete(m.params.requestId);}
 if(m.method==='Network.loadingFinished')requestUrls.delete(m.params.requestId);
